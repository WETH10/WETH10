...
contract WETH8 is IWETH8 {

    string public constant name = "Wrapped Ether v8";
    string public constant symbol = "WETH8";
    ...
    /// @dev Records amount of WETH8 token owned by account.
    ...
    /// @dev Records number of WETH8 token that account (second) will be allowed to spend on behalf of another account (first) through {transferFrom}.
    mapping (address => mapping (address => uint256)) public override allowance;

    /// @dev Current amount of flash-minted WETH8 token.
    ...
    /// @dev Returns the total supply of WETH8 token as the ETH held in this contract.
    function totalSupply() external view override returns (uint256) {
        return address(this).balance + flashMinted;
    }

    /// @dev Fallback, `msg.value` of ETH sent to this contract grants caller account a matching increase in WETH8 token balance.
    ...
    /// @dev Return the fee (zero) for flash lending an amount of WETH8 token.
    ...
    /// @dev Flash lends `value` WETH8 token to the receiver address.
    /// By the end of the transaction, `value` WETH8 token will be burned from the receiver.
    /// The flash-minted WETH8 token is not backed by real ETH, but can be withdrawn as such up to the ETH balance of this contract.
    ...
    /// WETH8 token implementation adapted from https://github.com/albertocuestacanada/ERC20Permit/blob/master/contracts/ERC20Permit.sol.
    ...
    /// @dev Moves `value` WETH8 token from account (`from`) to account (`to`) using allowance mechanism.
